  ...
  driftMists = function() {
    for (var iMist = 0; iMist < Mists.length; iMist++) {
      ...
      if (mistObj.flashProgress > 0) {
        ...
        if (mistObj.flashProgress < FLASH_DURATION_FRAMES / 2) {
          mistObj.flashInstance.opacity =
              mistObj.flashProgress / FLASH_DURATION_FRAMES;
        } else {
          mistObj.flashInstance.opacity = 
              Math.sin(Math.PI * 
                  (mistObj.flashProgress / FLASH_DURATION_FRAMES));
        ...
        mistObj.flashProgress++;
        if (mistObj.flashProgress > FLASH_DURATION_FRAMES) {
          mistObj.flashProgress = 0;
          mistObj.flashInstance.opacity = 0;
        }
      }
...
(function() {
  ...
  for (var y = 0; y < 1; y += STEP_Y) {
    for (var x = 0; x < 1; x += STEP_X) {
      var vertexObj = {};
      
      vertexObj.key = '(' + x + ',' + y + ')';
      vertices[vertexObj.key] = vertexObj;
      
      vertexObj.pointFrom = new Point(x, y);
      
      vertexObj.lines = {};
      
      vertexObj.lines.diagBack =
          new Path.Line(vertexObj.pointFrom,
              new Point(x - STEP_X, y - STEP_Y));
      vertexObj.lines.diagBack.strokeColor = 'green';
    }
  }

  scaleGrid = function() {
    for (var vxKey in vertices) {
      var vertexObj = vertices[vxKey];
